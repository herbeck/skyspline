...
typedef std::vector<double> xstate_type;
...
public:
	...
	void operator() ( const xstate_type &x , xstate_type &dxdt , double t)//, const double /* t */ )
	{
	...
	xstate_type get_statevec0(){
		xstate_type x(g_maxk+1, 0.);
	...
	double get_Lambda(xstate_type x){
		return x.at(i_Lambda());
	}
	vec get_g(xstate_type x){
		vec g = zeros<vec>(g_maxk);
		for (int i = 0; i < g_maxk; i++){
			g(i) = x.at(i);
		}
...
List solve_dgdt0(vec times, vec Fs, vec Ys
 ...
 , double treeT // actually length of time axis //TODO deprecate
	...
	xstate_type x = dgdt0.get_statevec0();
...
double solve_CoM12L(vec times, vec Fs, vec Ys
 ...
 , double A
	...
	xstate_type x = com12L.get_statevec0();
